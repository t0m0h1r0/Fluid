...
class PoissonMultigridSolver(PoissonSolverBase):
    ...
    def relax(u: jnp.ndarray, f: jnp.ndarray, dx: jnp.ndarray, num_iter: int, omega: float) -> jnp.ndarray:
    ...
    def prolong(coarse: jnp.ndarray, fine_shape: Tuple[int, ...]) -> jnp.ndarray:
        ...
        # x方向の補間
        fine = fine.at[1:-1:2, ::2, ::2].set(0.5 * (fine[:-2:2, ::2, ::2] + fine[2::2, ::2, ::2]))
        
        # y方向の補間
        fine = fine.at[:, 1:-1:2, ::2].set(0.5 * (fine[:, :-2:2, ::2] + fine[:, 2::2, ::2]))
