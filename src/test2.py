...
def create_grid(shape=(32, 32, 32)):
    ...
    return GridInfo(
        shape=shape,
        dx=(1.0 / (shape[0] - 1), 1.0 / (shape[1] - 1), 1.0 / (shape[2] - 1)),
    )
...
def test_continuity_linear_field():
    ...
    nonconservative_derivative = nonconservative_eq.compute_derivative(
    ...
    analytical_nonconservative = -(1 * gradient_x + 2 * gradient_y + 3 * gradient_z)

    # 保存形の検証
    assert np.allclose(
        conservative_derivative.data, analytical_conservative, rtol=1e-5, atol=1e-8
    )
...
def test_continuity_sin_field():
    ...
    # 複雑な速度場
    velocity = VectorField(
        grid, initial_values=(np.cos(coords[0]), -np.sin(coords[1]), np.exp(coords[2]))
    )
    ...
    print(
        conservative_eq.get_diagnostics(scalar_field, velocity, conservative_derivative)
    )

    print("\nNon-Conservative Derivative Diagnostics:")
    print(
        nonconservative_eq.get_diagnostics(
            scalar_field, velocity, nonconservative_derivative
        )
    )
...
def test_cfl_timestep():
    ...
    # 複雑な速度場
    velocity = VectorField(
        grid, initial_values=(np.sin(coords[0]), np.cos(coords[1]), np.exp(coords[2]))
    )
    ...
    # CFL条件による時間刻み幅を計算
    cfl_timestep = conservative_eq.compute_cfl_timestep(velocity, grid, cfl=0.5)
