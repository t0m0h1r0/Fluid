...
def compute_curvature(phi: np.ndarray, dx: float) -> np.ndarray:
    ...
    # 発散を計算して曲率を得る
    return sum(np.gradient(grad_normalized[i], dx, axis=i) 
              for i in range(phi.ndim))

def reinitialize(phi: np.ndarray, dx: float, dt: float = 0.1,
...
def extend_velocity(velocity: np.ndarray, phi: np.ndarray,
                   dx: float, n_steps: int = 5) -> np.ndarray:
    """界面の法線方向に速度場を拡張
    ...
    for _ in range(n_steps):
        ...
        # 法線方向の速度勾配をゼロにする
        result = result - dt * sum(normal[i] * vel_grad[i] 
                                 for i in range(phi.ndim))
