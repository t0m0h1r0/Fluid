...
このモジュールは、Level Set法における界面追跡のための場のクラスを定義します。
"""
...
class LevelSetParameters:
    ...
    Attributes:
        epsilon: 界面の厚さ
        min_value: 数値計算時の最小値（ゼロ除算防止用）
        reinit_interval: 再初期化の間隔（ステップ数）
        reinit_steps: 再初期化の反復回数
        reinit_dt: 再初期化時の時間刻み幅
    """
...
class LevelSetField(ConservedField):
    ...
    このクラスは、Level Set法による界面追跡のための場を表現します。
    ConservedFieldを継承し、質量保存を考慮します。
    """
    ...
    def need_reinit(self) -> bool:
        """再初期化が必要かどうかを判定"""
        return (self._steps_since_reinit >= 
                self.params.reinit_interval)
