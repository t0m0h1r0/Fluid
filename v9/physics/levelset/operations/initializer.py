...
class LevelSetInitializer(BaseLevelSetOperation):
    ...
    def initialize(self, shape: Tuple[int, ...], **kwargs) -> np.ndarray:
        ...
        background_phase = kwargs.get("background_phase")

        # 各オブジェクトの処理
        for obj in objects:
            obj_type = obj.get("type")
            obj_phase = obj.get("phase")
            ...
            if obj_phase != background_phase:
                phi = np.minimum(phi, phi_new)  # 異なる相のオブジェクトならminを取る
            else:
                phi = np.maximum(phi, phi_new)  # 背景相と同じ相ならmaxを取る
    ...
    def _compute_sphere_levelset(self, obj: Dict[str, Any]) -> np.ndarray:
        ...
        squared_distance = sum(
            (coord - c) ** 2 for coord, c in zip(self.coords, center)
        )
        distance = np.sqrt(squared_distance)
        
        # 符号付き距離関数を計算（正: 球の外側、負: 球の内側）
