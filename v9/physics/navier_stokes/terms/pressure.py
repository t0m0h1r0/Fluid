...
class PressureTerm(NavierStokesTerm):
    ...
    def compute_correction(self, velocity: VectorField,
                         pressure: ScalarField,
                         dt: float,
                         density: Optional[ScalarField] = None) -> VectorField:
        """速度場の圧力補正を計算
    ...
    def project_velocity(self, velocity: VectorField,
                        pressure: ScalarField,
                        density: Optional[ScalarField] = None) -> VectorField:
        """速度場を非圧縮条件に射影
    ...
    def get_diagnostics(self, velocity: VectorField,
        ...
        # 圧力勾配の大きさを計算
        grad_p = np.array([np.gradient(pressure.data, velocity.dx, axis=i) 
                          for i in range(velocity.ndim)])
        ...
        diag.update({
            ...
            'pressure_l2norm': np.sqrt(np.sum(pressure.data**2) * velocity.dx**velocity.ndim)
        })
        return diag