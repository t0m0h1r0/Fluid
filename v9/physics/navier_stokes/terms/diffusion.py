...
class DiffusionTerm(NavierStokesTerm):
        ...
        **kwargs,
        ...
        for i, v_i in enumerate(velocity.components):
            if self.use_conservative:
                ...
                # ∇μ・(∇u + ∇u^T) の計算
                grad_mu = np.array(
                    [np.gradient(mu, dx, axis=j) for j in range(velocity.ndim)]
                )
    ...
    def get_diagnostics(
        ...
        **kwargs,
        ...
        diag = super().get_diagnostics(
            velocity, viscosity=viscosity, density=density, **kwargs
        )
    ...
    def _compute_stress_tensor(
        self, velocity: VectorField, viscosity: ScalarField = None
    ) -> np.ndarray:
        """粘性応力テンソルを計算"""
        dx = velocity.dx
        ndim = velocity.ndim
    ...
    def _compute_dissipation(
        self, velocity: VectorField, stress_tensor: np.ndarray
    ) -> float:
        """粘性散逸を計算"""
        dx = velocity.dx
        ndim = velocity.ndim
