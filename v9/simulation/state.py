...
このモジュールは、シミュレーションの状態（フィールドや時間など）を
カプセル化して管理します。
"""
...
class SimulationState:
    ...
    def save(self, directory: Path):
        ...
        # フィールドデータの保存
        np.save(directory / 'velocity.npy',
                [v.data for v in self.velocity.components])
    ...
    def update_statistics(self):
        """シミュレーション統計を更新"""
        # 速度場の統計
        vel_mag = self.velocity.magnitude().data
        self.statistics.update({
            'max_velocity': np.max(vel_mag),
            'mean_velocity': np.mean(vel_mag),
            'max_pressure': np.max(np.abs(self.pressure.data)),
            'kinetic_energy': 0.5 * np.sum(vel_mag**2) * self.velocity.dx**self.velocity.ndim,
            'interface_area': self.levelset.compute_area(),
            'phase1_volume': np.sum(self.levelset.heaviside()) * self.velocity.dx**self.velocity.ndim
        })
        
        # 発散チェック
        div = self.velocity.divergence()
        self.statistics['max_divergence'] = np.max(np.abs(div))
