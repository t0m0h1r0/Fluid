...
class SimulationMonitor:
    ...
    def __init__(self, config: Dict[str, Any], logger: SimulationLogger):
        ...
        # 統計情報の保存先
        self.stats_file = Path(config['visualization']['output_dir']) / 'statistics.json'
    ...
    def _check_anomalies(self, state: SimulationState):
        ...
        if len(self.history['kinetic_energy']) > 1:
            energy_ratio = (state.statistics['kinetic_energy'] /
                          self.history['kinetic_energy'][0])
    ...
    def _update_performance_stats(self, current_time: float):
        ...
        if self.update_count % 10 == 0:
            self.logger.info(
                f"Performance: {steps_per_second:.1f} steps/s"
            )
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'energy_ratio': (self.history['kinetic_energy'][-1] /
                           self.history['kinetic_energy'][0])
        }
    ...
    def plot_history(self, output_dir: Path):
        ...
        try:
            ...
            plt.plot(self.history['time'], self.history['kinetic_energy'],
            ...
            # 発散のプロット
            plt.figure(figsize=(10, 6))
            plt.semilogy(self.history['time'], self.history['max_divergence'],
            ...
            # 体積保存のプロット
            plt.figure(figsize=(10, 6))
            volume_ratio = np.array(self.history['phase1_volume']) / self.history['phase1_volume'][0]
