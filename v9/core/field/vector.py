...
class VectorField:
    ...
    def curl(self) -> Optional['VectorField']:
        ...
        # ∂w/∂y - ∂v/∂z
        result.components[0].data = np.gradient(w, self.dx, axis=1) - \
                                  np.gradient(v, self.dx, axis=2)
        # ∂u/∂z - ∂w/∂x
        result.components[1].data = np.gradient(u, self.dx, axis=2) - \
                                  np.gradient(w, self.dx, axis=0)
        # ∂v/∂x - ∂u/∂y
        result.components[2].data = np.gradient(v, self.dx, axis=0) - \
                                  np.gradient(u, self.dx, axis=1)
    ...
    def dot(self, other: 'VectorField') -> ScalarField:
        ...
        result = ScalarField(self.shape, self.dx)
        result.data = sum(c1.data * c2.data 
