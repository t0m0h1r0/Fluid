...
def parse_args():
    ...
    parser.add_argument("--config", type=str, default="config.yaml",
                       help="設定ファイルのパス")
    parser.add_argument("--output", type=str, default="pressure_test",
                       help="出力ディレクトリ名")
...
def save_pressure_field(pressure, iteration: int, output_dir: Path):
    ...
    plt.imshow(pressure[:, :, slice_idx].T, origin='lower',
...
def main():
    ...
    # ロガーの設定
    logger = SimulationLogger("PressureTest",
                            LogConfig(level="info", log_dir=output_dir))
    ...
    try:
        ...
        for iteration in range(1, args.max_iter + 1):
            ...
            new_pressure = poisson_solver.solve(
                initial_solution=state.pressure.data,
                rhs=rhs.data,
                dx=state.velocity.dx
            )
