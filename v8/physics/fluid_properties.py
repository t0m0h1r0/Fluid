...
class FluidProperties:
    ...
    def _validate_phases(self):
        ...
        for name, phase in self.phases.items():
            if phase.density <= 0:
                raise ValueError(f"相 {name} の密度は正である必要があります")
            if phase.viscosity <= 0:
                raise ValueError(f"相 {name} の粘性係数は正である必要があります")
            if phase.surface_tension is not None and phase.surface_tension < 0:
                raise ValueError(f"相 {name} の表面張力係数は非負である必要があります")
    ...
    def _get_property(self, phi: Field, 
                     property_func: callable) -> np.ndarray:
        """指定された物性値の場を取得"""
        # キャッシュキーの生成
