...
class SimulationVisualizer:
    ...
    def create_visualization(self, data: Dict[str, np.ndarray],
                           timestep: int,
                           save: bool = True) -> None:
        """複合可視化の作成"""
        # マルチプロット図の作成
        ...
        # 全体のタイトル設定
        fig.suptitle(f'シミュレーション時間: {data["stats"]["total_time"]:.3f}s\n'
                    f'タイムステップ: {timestep}')
    ...
    def _plot_velocity_field(self, fig: plt.Figure,
        ...
        if 'velocity' in data:
            velocity_magnitude = np.sqrt(
                sum(v**2 for v in data['velocity'])
            )
    ...
    def _setup_axis_labels(self, ax: plt.Axes, 
                          xlabel: str = 'X', 
                          ylabel: str = 'Y',
                          zlabel: str = None):
        """軸ラベルの設定"""
