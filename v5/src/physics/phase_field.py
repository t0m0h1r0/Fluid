...
class PhaseFieldSolver:
    ...
    def initialize_field(
        self, shape: Tuple[int, ...], domain_size: Tuple[float, ...]
    ) -> np.ndarray:
        """相場の初期化"""
    ...
    def add_layer(self, field: np.ndarray, layer: Layer) -> np.ndarray:
        ...
        if self._grid is None:
            raise RuntimeError(
                "グリッドが初期化されていません。initialize_fieldを先に呼び出してください。"
            )
    ...
    def advance(
        self, phi: np.ndarray, velocity: List[np.ndarray], dt: float
    ) -> np.ndarray:
        """改良された時間発展の計算"""
        # 移流項
    ...
    def _compute_advection(
        self, phi: np.ndarray, velocity: List[np.ndarray]
    ) -> np.ndarray:
        """WENOスキームを用いた移流項の計算"""
    ...
    def compute_chemical_potential(self, phi: np.ndarray) -> np.ndarray:
        ...
        # 安定化項
        stabilization = self._stab_coeff * (
            np.maximum(phi - 1.0, 0.0) - np.minimum(phi + 1.0, 0.0)
        )
