...
class NavierStokesSolver:
    ...
    def compute_timestep(
        ...
        dt_surface = np.sqrt(
            min_density * dx**3 / (2 * np.pi * surface_tension + 1e-10)
        )

        return min(dt_convection, dt_diffusion, dt_surface)

    def compute_convection(
        self, velocity: List[np.ndarray], density: np.ndarray, axis: int
    ) -> np.ndarray:
        """WENOスキームを用いた移流項の計算"""
    ...
    def pressure_projection(
        self, velocity: List[np.ndarray], density: np.ndarray, dt: float
    ...
    def runge_kutta4(
        ...
        viscosity: np.ndarray,
        dt: float,
        external_forces: Optional[List[np.ndarray]] = None,
    ) -> List[np.ndarray]:
        """4次のルンゲクッタ法による時間積分"""
        ...
        def compute_rhs(v: List[np.ndarray], rho: np.ndarray) -> List[np.ndarray]:
            ...
            for axis in range(len(v)):
                ...
                # 全項の合計
                total = (
                    -advection + viscosity * diffusion / rho + gravity + external_force
                )
                rhs.append(total)
    ...
    def compute_laplacian(self, field: np.ndarray) -> np.ndarray:
        ...
        for axis in range(field.ndim):
            ...
            # 2階微分の近似
            laplacian += np.roll(field, -1, axis) - 2 * field + np.roll(field, 1, axis)
