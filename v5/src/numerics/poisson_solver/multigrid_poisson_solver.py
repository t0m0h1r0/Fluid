...
class MultigridPoissonSolver(AbstractPoissonSolver):
                    ...
                    for k in range(coarse_grid.shape[2]):
                        ...
                        if i_f + 1 < fine_shape[0]:
                            fine_grid[i_f + 1, j_f, k_f] = 0.5 * (
                                coarse_grid[i, j, k]
                                + coarse_grid[
                                    min(i + 1, coarse_grid.shape[0] - 1), j, k
                                ]
                            )
                        if j_f + 1 < fine_shape[1]:
                            fine_grid[i_f, j_f + 1, k_f] = 0.5 * (
                                coarse_grid[i, j, k]
                                + coarse_grid[
                                    i, min(j + 1, coarse_grid.shape[1] - 1), k
                                ]
                            )
                        if k_f + 1 < fine_shape[2]:
                            fine_grid[i_f, j_f, k_f + 1] = 0.5 * (
                                coarse_grid[i, j, k]
                                + coarse_grid[
                                    i, j, min(k + 1, coarse_grid.shape[2] - 1)
                                ]
                            )
    ...
    def _smooth(
        self, u: np.ndarray, f: np.ndarray, iterations: int, dx: float
    ) -> np.ndarray:
        """ガウス-ザイデル法によるスムージング"""
    ...
    def _v_cycle(
        self, u: np.ndarray, f: np.ndarray, level: int, dx: float
    ) -> np.ndarray:
        """V-サイクルの実行"""
