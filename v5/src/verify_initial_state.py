...
from numerics.poisson_solver.multigrid_poisson_solver import MultigridPoissonSolver as PoissonSolver
...
def verify_density_field(density: np.ndarray, config: SimulationConfig, X: np.ndarray, Y: np.ndarray, Z: np.ndarray):
    """密度場が設定通りになっているか検証する"""
    # 各相の密度を取得
    ...
    for layer in config.layers:
        ...
        if len(z_indices) > 0:
            ...
            print(f"  Actual density range: [{layer_density.min():.2f}, {layer_density.max():.2f}]")
            ...
            if np.max(density_diff) > 1e-6:
                print(f"  WARNING: Maximum density deviation: {np.max(density_diff):.2e}")
        else:
    ...
    for sphere in config.spheres:
        ...
        if np.sum(mask) > 0:
            ...
            print(f"  Expected density: {expected_density}")
            print(f"  Actual density range: [{sphere_density.min():.2f}, {sphere_density.max():.2f}]")
...
def main():
    ...
    for sphere in config.spheres:
        ...
        mask = r <= sphere.radius
        density[mask] = phase_density
        print(f"Sphere {sphere.phase} set: center={sphere.center}, radius={sphere.radius}, density={phase_density}")
