...
class DataWriter:
    ...
    def _save_field_visualization(
        self, field: np.ndarray, field_type: str, base_filename: str
    ):
        """フィールドの可視化を保存"""
        # 2D断面プロット
        ...
        # 3つの断面でのプロット
        for i, (axis, plane) in enumerate([(2, "xy"), (1, "xz"), (0, "yz")]):
            ax = fig.add_subplot(1, 3, i + 1)
            slice_data = self._get_central_slice(field, axis)

            im = ax.imshow(
                slice_data.T, cmap=cmap, norm=norm, origin="lower", aspect="equal"
            )
        ...
        # プロットの保存
        plot_filename = os.path.join(self.plot_dir, f"{base_filename}_{field_type}.png")
    ...
    def save_metadata(self, metadata: dict, step: int):
        """メタデータの保存"""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = os.path.join(self.data_dir, f"metadata_{step:06d}_{timestamp}.h5")
