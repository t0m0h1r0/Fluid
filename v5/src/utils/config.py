...
class SimulationConfig:
    def __init__(self, config_file: str):
        ...
        self.layers = [
            LayerConfig(**layer) for layer in config["initial_condition"]["layers"]
        ]
        self.spheres = [
            SphereConfig(**sphere) for sphere in config["initial_condition"]["spheres"]
        ]
        ...
        self.surface_tension = config["physical"].get(
        ...
        self.max_steps = numerical_config.get(
    ...
    def validate(self) -> List[str]:
        ...
        for sphere in self.spheres:
            if not (
                0 <= sphere.center[0] <= self.Lx
                and 0 <= sphere.center[1] <= self.Ly
                and 0 <= sphere.center[2] <= self.Lz
            ):
