...
class ParallelCompute:
    ...
    def parallel_gradient(
        field: np.ndarray, axis: int, spacing: float = 1.0
    ) -> np.ndarray:
        """並列化された勾配計算
        ...
        for idx in prange(total_size):
            ...
            for i in range(2, n - 2):
                grad_line[i] = (
                    -line[i + 2] + 8 * line[i + 1] - 8 * line[i - 1] + line[i - 2]
                ) / (12 * spacing)

            # 境界での4次精度片側差分
            # 前方境界
            grad_line[0] = (
                -25 * line[0] + 48 * line[1] - 36 * line[2] + 16 * line[3] - 3 * line[4]
            ) / (12 * spacing)
            grad_line[1] = (
                -3 * line[0] - 10 * line[1] + 18 * line[2] - 6 * line[3] + line[4]
            ) / (12 * spacing)
            ...
            grad_line[-2] = (
                3 * line[-1] + 10 * line[-2] - 18 * line[-3] + 6 * line[-4] - line[-5]
            ) / (12 * spacing)
    ...
    def parallel_compute_field(
        ...
        **kwargs,
        ...
        with multiprocessing.Pool(processes=num_cores) as pool:
            processed_chunks = pool.map(partial(func, **kwargs), chunks)

        # 結果の結合を最適化
        return np.concatenate(processed_chunks)

    @staticmethod
    def create_parallel_operator(
        func: Callable, chunk_size: int = None, **kwargs
    ) -> Callable:
        """任意の関数を並列化可能な演算子に変換
        ...
        def parallel_operator(field: np.ndarray) -> np.ndarray:
            return ParallelCompute.parallel_compute_field(
                partial(func, **kwargs), field, chunk_size=chunk_size
            )
    ...
    def static_gradient(
        ...
        for idx in np.ndindex(
            tuple(
                s.stop - s.start if isinstance(s, slice) else s
        ...
        # 前方境界
        slices[axis] = 0
        gradient[tuple(slices)] = compute_forward_boundary(field, axis, slices, spacing)
...
@njit
def compute_forward_boundary(
    field: np.ndarray, axis: int, slices: list, spacing: float
) -> np.ndarray:
    """前方境界での4次精度片側差分"""
    idx = list(slices)
