...
from physics.phase_field import PhaseFieldSolver
...
class SimulationManager:
    ...
    def _setup_logger(self) -> logging.Logger:
        ...
        log_file = os.path.join(
            self.log_dir, f"simulation_{datetime.now():%Y%m%d_%H%M%S}.log"
        )
    ...
    def advance_timestep(self, adaptive_dt: bool = True) -> Dict[str, np.ndarray]:
        ...
        try:
            ...
            velocity_new = self._corrector_step(
                velocity_pred, pressure_new, density, dt
            )
    ...
    def _predictor_step(
        self, density: np.ndarray, viscosity: np.ndarray, dt: float
    ) -> List[np.ndarray]:
        """予測子ステップ：移流拡散方程式の解法"""
        # 外力項の計算（重力と表面張力）
        ...
        velocity_pred = self.ns_solver.runge_kutta4(
            self.velocity, density, viscosity, dt, external_forces=external_forces
        )

        return velocity_pred

    def _pressure_correction(
        self, velocity_pred: List[np.ndarray], density: np.ndarray, dt: float
    ) -> np.ndarray:
        ...
        pressure = self.ns_solver.poisson_solver.solve(
            rhs, tolerance=self.pressure_tolerance, max_iterations=self.max_iterations
        )
    ...
    def _update_phase_field(self, velocity: List[np.ndarray], dt: float) -> np.ndarray:
        """Phase-Field方程式の時間発展"""
    ...
    def _compute_adaptive_timestep(self) -> float:
        ...
        dx = min(
            self.config.Lx / self.config.Nx,
            self.config.Ly / self.config.Ny,
            self.config.Lz / self.config.Nz,
        )

        dt = self.ns_solver.compute_timestep(self.velocity, density, viscosity, dx)
    ...
    def _check_convergence(
        self, velocity: List[np.ndarray], pressure: np.ndarray
    ) -> bool:
        """収束判定"""
        # 速度の変化
        velocity_change = max(
        ...
        return (
            velocity_change < self.velocity_tolerance
            and pressure_change < self.pressure_tolerance
        )
    ...
    def _update_statistics(self, dt: float):
        ...
        self.stats.interface_energy = self.phase_solver.compute_interface_energy(
            self.phi
        )
